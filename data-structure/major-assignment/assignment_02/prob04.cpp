...
void search_words(string word_set) {
	...
	// tota = value들의 합으로 헤서 다 등장한 경우에 출력
	// 권오흠 교수님의 bool 조언을 받고 아래 코드처럼 수정함
	...
	for (auto p : directory) {
		bool check = true;  // 모든 검색어들 중 하나라도 등장하지 않았다면 false
		for (auto i : words)
			i.second = 0;  // 이전 사람의 정보에서 검색어의 등장여부로 갱신되어 있으므로 0으로 다시 초기화

		for (auto i : words) {
			string combined_info = p.name + " " + p.address + " " + p.phone + " " + p.email + " " + p.web;  // combined_info에 사람의 모든 정보를 합해 문자열로 만듦
			if (combined_info.find(i.first) == string::npos)  // 검색어가 사람의 정보 문자열에 등장하지 않는다면
				check = false;  // check를 false로 갱신 
		}

		if (check)  // 모든 검색어가 전부 등장했다면
			print_person(p);  // 해당 사람의 정보를 출력 
	}
}
